...
for seed in seeds:
    ...
    for dset in keys:
        ...
        SAVE_PATH = f"saves/{seed}/{TASK}/"
        device_used = "cuda:0" if torch.cuda.is_available() else "cpu"

        models = []
        model_names = []
        for i in range(0,25, 5):
            name = f"google/multiberts-seed_{i}"
            model_names.append("multiberts_seed_" + str(i))
            models.append(name)
        ...
        if os.path.exists(SAVE_PATH):
            ...
            if not os.path.exists(f"saves/{seed}/"):
                os.mkdir(f"saves/{seed}/")
            if not os.path.exists(f"saves/{seed}/{TASK}/"):
                os.mkdir(SAVE_PATH)
